...
const SingleShow = ({id,title,backdrop_path,poster_path,overview,release_date,name,first_air_date}) => {
    const [like,setLike]=useState(false)
    const [show,setShow]=useState(false)
    const {user,type}=useGlobalContext()
    const showId=doc(db,'users',`${user?.email}`)

    const correctTitle = type==='tv'?name:title
    const date = type==='tv'?first_air_date:release_date 
    ...
    <div onClick={_=>setShow(p=>!p)} onMouseEnter={_=>setShow(true)} onMouseLeave={_=>setShow(false)} className='w-[120px] md:w-[180px] lg:w-[290px] relative p-4 mx-auto z-40'>
        <div className={`flex items-center justify-center absolute top-0 left-0 h-full transition duration-500 w-full ${show && 'bg-black bg-opacity-[80%]'}`}>
            <h1 className={`${show?'opacity-1 translate-y-0':'opacity-0 translate-y-[50%]'} w-[80%] text-center text-lg md:text-xl font-bold  text-white transition duration-500`}>{correctTitle}</h1>
